let object = [
  ...
  nextBtn.addEventListener("click", () => {
    ...
    if (object.length - 10 > currentItem) {
      currentItem += 10;
      page.innerHTML = currentItem / 10 + 1;
      displayEmployees(currentItem);
    } else {
  ...
  lastBtn.addEventListener("click", () => {
    if (currentItem < object.length - 10) {
      currentItem = object.length - 10;
      page.innerHTML = currentItem / 10 + 1;
      displayEmployees(currentItem);
    } else {
      alert("You are already in last page");
  ...
  function displayEmployees(start) {
    ...
    for (let i = start; i < start + 10 && object.length; i++) {
      add += `
      
      
      <tr>
        
         <td id="para">${object[i].id}</td>
      
      
         
         <td id="name">${object[i].name}</td>
      
    
         
         <td id="email">${object[i].email}</td>
      </tr>
